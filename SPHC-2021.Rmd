...
#### 3.2. Multiple imputation and survey analyses
##### 3.2.1. Multiple imputation
###### 3.2.1.1. Check calibrated weights and proxy auxiliary variables
```{r}
...
###### 3.2.1.2. Obtain incomplete dataset
```{r}
summary( d_2021 )
...
d_2021_incomplete_prop <- d_2021 %>% 
  ...
  mutate( 
    country_of_birth = factor( 
      ...
      levels = c( "Sweden", "Europe", "Outside Europe" ) ),
    education = relevel( education, ref = ">=13 years" ),
    marital_status = relevel( marital_status, ref = "Currently married" ) 
    )

# create weight strata (n = 25) for two-level multivariate normal imputation
d_2021_incomplete_prop$weight_strata <- as.factor( cut( d_2021_incomplete_prop$calibrated_weight,
                                                   breaks = quantile( d_2021_incomplete_prop$calibrated_weight, 
                                                                      probs = seq( 0, 1, length.out = 26 ) ),
                                                   labels = 1:25,
                                                   include.lowest = TRUE,
                                                   right = TRUE ) )

ggplot( d_2021_incomplete_prop %>% 
...
summary( d_2021_incomplete_prop )
sapply( d_2021_incomplete_prop, class ) # all continuous variables are numeric, and all categorical variables are factor
save( d_2021_incomplete_prop, file = "d_2021_incomplete_prop.RData" )
```

###### 3.2.1.3. Visualize missing pattern
```{r}
# identify missing variables
vis_miss( d_2021_incomplete_prop )
n_miss( d_2021_incomplete_prop )
miss_var_summary( d_2021_incomplete_prop ) # 2.2% missing in sexual_identity_2021, 2.1% in education, and 0.2% in income
```

###### 3.2.1.4. Two-level multivariate normal imputation
```{r}
# use multiple imputation to account for item non-response
# assume Missing At Random (MAR)

# specify imputation model
fml_imp_2021_prop <- sexual_identity_2021 + education + income ~ 1 + age*sex + country_of_birth + marital_status + ( 1 | weight_strata )

# perform a "dry run"
imp_ini_2021_prop <- jomoImpute( data = d_2021_incomplete_prop, # incomplete dataset
                                 formula = fml_imp_2021_prop, # imputation model
...
summary( imp_ini_2021_prop ) # check output
...
imp_preliminary_2021_prop <- jomoImpute( data = d_2021_incomplete_prop,
                                         formula = fml_imp_2021_prop,
                                         random.L1 = "full",
                                         n.burn = 5000,
                                         n.iter = 1000,
                                         m = 2,
...
save( imp_preliminary_2021_prop, file = "imp_preliminary_2021_prop.RData" )
summary( imp_preliminary_2021_prop ) # summarize model and display convergence statistics
plot( imp_preliminary_2021_prop, trace = "all", print = "beta" ) # check trace and autocorrelation plots


# final imputation with the chosen number of iterations
imp_final_2021_prop <- jomoImpute( data = d_2021_incomplete_prop,
                                   formula = fml_imp_2021_prop,
                                   random.L1 = "full",
                                   n.burn = 2000,
                                   n.iter = 1000,
...
save( imp_final_2021_prop, file = "imp_final_2021_prop.RData" )
summary( imp_final_2021_prop )
plot( imp_final_2021_prop, trace = "all", print = "beta" )
```

###### 3.2.1.5. Validate imputed data
```{r}
# extract imputed datasets
original_data_2021_prop <- mitmlComplete( imp_final_2021_prop, print = 0 ) # extract original incomplete dataset
implist_2021_prop <- mitmlComplete( imp_final_2021_prop, print = "all" ) # extract all imputed datasets

original_data_2021_prop$imputation <- "0"
all_data_2021_prop <- bind_rows( original_data_2021_prop, 
                                 bind_rows( implist_2021_prop, .id = "imputation" ) ) # merge datasets
all_data_2021_prop$imputation <- as.numeric( all_data_2021_prop$imputation )
summary( all_data_2021_prop )

# sexual identity in 2021
ggplot( all_data_2021_prop[ !is.na( all_data_2021_prop$sexual_identity_2021 ), ],
...
# education
ggplot( all_data_2021_prop[ !is.na( all_data_2021_prop$education ), ],
        aes( fill = factor( education, levels = c( "<=9 years", "10-12 years",  ">=13 years" ) ), x = imputation ) ) + 
  geom_bar( position = "fill" ) + 
  scale_y_continuous( labels = scales::percent ) + 
  labs(
    x = "Imputation number",
...
# income
summary( all_data_2021_prop$income )
nrow( all_data_2021_prop[ all_data_2021_prop$income < 0 & !is.na( all_data_2021_prop$income ), ] ) # 181 imputed values are negative
all_data_2021_prop$income <-ifelse( all_data_2021_prop$income < 0, 0, all_data_2021_prop$income )
```

##### 3.2.2. Survey analyses
###### 3.2.2.1. Define survey design
```{r}
...
survey_design_imp <- svydesign( ids = ~ 1, 
                                ...
                                weights = ~ calibrated_weight,
                                fpc = ~ no.of.population,
                                data = imputationList( implist_2021 )
                                ) # without pre-calibration

results <- with( 
  survey_design_imp, svyby( formula = as.formula( ~ I( sexual_identity_2021 == "Heterosexual" ) ),
                            by = ~ sex,
                            FUN = svyciprop,
                            method = "beta" ) 
  )
