...
```{r}
summary( d_2021 )
selected_variables <- c( "sampling_strata_region", "no.of.population", "sample_size", "calibrated_weight", "sexual_identity_2021", "age", "sex", "country_of_birth", "utbniva2021", "occupation", "dispink2021", "marital_status" )
  
d_2021_incomplete <- d_2021 %>% 
  select( all_of( selected_variables ) ) %>%
  mutate( utbniva2021 = as.factor( utbniva2021 ) ) %>%
  rename( 
    education = utbniva2021,
    income = dispink2021,
    place_of_residence = sampling_strata_region
    )

# visualize the distribution of income (semi-continuous)
ggplot( d_2021_incomplete, aes( x = income ) ) +
  geom_histogram( binwidth = 10, fill = "steelblue", color = "steelblue" ) +
  xlim( c( 0, 20000 ) )

ggplot( d_2021_incomplete, aes( x = log( income ) ) ) +
  geom_histogram( binwidth = 0.01, fill = "steelblue", color = "steelblue" ) +
  xlim( c( log( 1 ), log( 20000 ) ) )

# generate two variables for income
d_2021_incomplete$income_cat <- as.factor( ifelse( d_2021_incomplete$income == 0, 0, 1 ) )
d_2021_incomplete$income_log_con <- ifelse( d_2021_incomplete$income == 0, NA, log( d_2021_incomplete$income ) )
d_2021_incomplete <- d_2021_incomplete %>% select( -c( income ) )
...
# create weight strata for two-level multivariate normal imputation
...
miss_var_summary( d_2021_incomplete ) # 31.6% missing in occupation, 2.2% in sexual_identity_2021, 2.1% in education, and 0.2% in income
```
...
# introduce random intercepts
fml_imp <- sexual_identity_2021 + education + income_cat + income_log_con ~ 1 + age + sex + age*sex + country_of_birth + occupation + marital_status + place_of_residence ( 1 | weight_strata )

# perform a "dry run"
ini <- jomoImpute( data = d_2021_incomplete, # incomplete dataset
                   formula = fml_imp, # imputation model
...
# check trace plot and autocorrelation plots
# to determine the number of burn-in and between-imputation iterations for the final imputation
imp_preliminary <- jomoImpute( data = d_2021_incomplete,
                               formula = fml_imp,
                               random.L1 = "full",
                               n.burn = 5000,
                               n.iter = 1000,
...
summary( imp_preliminary )
plot( imp_preliminary ) # display convergence and autocorrelation plots


# final imputation with the chosen number of iterations
imp_final <- jomoImpute( data = d_2021_incomplete,
                               formula = fml_imp,
                               random.L1 = "full",
                               n.burn = 5000,
                               n.iter = 100,
...
# validate imputation
summary( imp_final ) # summarize model and display convergence statistics
plot( imp_final ) # display convergence plots and autocorrelation plot

# extract list of imputed datasets
implist <- mitmlComplete( imp_final, "all" )
```

##### 3.2.2. Analyses using survey weights
```{r}
