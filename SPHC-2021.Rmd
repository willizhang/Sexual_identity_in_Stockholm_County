...
implist_2021_prop_transformed <- lapply( implist_2021_prop, function( df ) {
  within( df, {
    ...
    # marital status
    marital_status_exp = marital_status
    marital_status_conf = marital_status
    } )
  } )

imputed_data_2021_prop_transformed <- bind_rows( implist_2021_prop_transformed, .id = "imputation" ) # merge datasets
imputed_data_2021_prop_transformed$imputation <- as.numeric( imputed_data_2021_prop_transformed$imputation )
summary( imputed_data_2021_prop_transformed )
n_miss( imputed_data_2021_prop_transformed ) # 0 missing


# use calibrated weights to account for unit non-response
# create survey design
survey_design_imp_prop <- svydesign( ids = ~ 1, 
                                     strata = ~ sampling_strata_region,
                                     weights = ~ calibrated_weight,
                                     fpc = ~ no.of.population,
                                     data = imputationList( implist_2021_prop_transformed )
                                     ) # without pre-calibration
```

##### 5.2.2. Calculate proportions of sexual identities
```{r}
# among demographic subgroups
sexual_identities <- c( "Heterosexual", "Homosexual", "Bisexual", "None of the above" )
demographic_vars <- c( "age_exp", "generation", "sex_exp", "age_sex", "country_of_birth_exp", "education_exp", "income_exp", "marital_status_exp" )

prop_imp_list_subgroup_2021 <- list()

for( cat in demographic_vars ) {
  results <- calc_prop_imp_subgroup_2021( design = survey_design_imp_prop,
...
# in Stockholm County
prop_imp_summary_overall_2021 <- calc_prop_imp_overall_2021( design = survey_design_imp_prop,
                                                             sexual_identities = sexual_identities )

# merge results
prop_imp_summary_2021 <- rbind( prop_imp_summary_subgroup_2021, prop_imp_summary_overall_2021 )
