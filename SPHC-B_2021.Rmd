...
# a variable indicating missing values
d_2021_cc_model <- d_2021_cc_model %>% 
  mutate( 
    nomiss = !is.na( sexual_identity ) & !is.na( age_conf ) & !is.na( sex_conf ) & 
      !is.na( country_of_birth_conf ) & !is.na( education_conf ) & !is.na( income_conf ) & 
      !is.na( marital_status_conf ) & !is.na( place_of_residence_conf )
    )


##### create survey design #####
survey_design_cc_model <- subset(
  ...
  # restrict to only individuals with complete data on all variables included in the model
  nomiss )
...
##### 4.1.3. Fit Poisson, log-binomial, and logistic regression models
```{r}
# fit the three models for each exposure and each outcome
...
refit_model_cc_age <- list()

for ( name in names( fml_list_age ) ) {
  refit_model_cc_age[[ name ]] <- fit_models( fml_list_age[[ name ]],
                                              survey_design_cc_model )
  }
```
...
for ( outcome in outcomes_age ) {
  for ( mod in models ) {
    key <- paste( outcome, mod, sep = "_" )
    model <- refit_model_cc_age[[ outcome ]][[ mod ]]
    
    if ( !is.null( model ) ) {
    results_cc_age_coef_female[[ key ]] <- cbind( coef( model )[ exposures_age_female ], 
                                                  confint( model, ddf = degf( survey_design_cc_model ) )[ exposures_age_female, , drop = FALSE ] )
    }
  }
}
...
results_cc_age_pr_poisson_male <- extract_results_pr( results_cc_age_coef_male, "poisson", identities, "age" ) %>%
    mutate( Exposure = ifelse( grepl( "age_exp", Exposure ) & !grepl( "Female", Exposure ), 
                               paste0( Exposure, "_Male" ), Exposure ) )

results_cc_age_pr_poisson_female <- extract_results_pr( results_cc_age_coef_female, "poisson", identities, "age" ) %>%
    mutate( Exposure = ifelse( grepl( "age_exp", Exposure ) & !grepl( "Male", Exposure ), 
                               paste0( Exposure, "_Female" ), Exposure ) )

results_cc_age_pr_poisson <- rbind( results_cc_age_pr_poisson_male[ c( 1:3 ), ], 
                                    results_cc_age_pr_poisson_female[ c( 1:3 ), ] )

# results from log binomial regression
results_cc_age_pr_log_binomial_male <- extract_results_pr( results_cc_age_coef_male, "log_binomial", identities, "age" ) %>%
  mutate( Exposure = ifelse( grepl( "age_exp", Exposure ) & !grepl( "Female", Exposure ), 
                             paste0( Exposure, "_Male" ), Exposure ) )

results_cc_age_pr_log_binomial_female <- extract_results_pr( results_cc_age_coef_female, "log_binomial", identities, "age" ) %>%
  mutate( Exposure = ifelse( grepl( "age_exp", Exposure ) & !grepl( "Male", Exposure ), 
                             paste0( Exposure, "_Female" ), Exposure ) )

results_cc_age_pr_log_binomial <- rbind( results_cc_age_pr_log_binomial_male[ c( 1:3 ), ], 
                                    results_cc_age_pr_log_binomial_female[ c( 1:3 ), ] )

# results from logistic regression
results_cc_age_pr_logistic_male <- extract_results_pr( results_cc_age_coef_male, "logistic", identities, "age" ) %>%
  mutate( Exposure = ifelse( grepl( "age_exp", Exposure ) & !grepl( "Female", Exposure ), 
                             paste0( Exposure, "_Male" ), Exposure ) )

results_cc_age_pr_logistic_female <- extract_results_pr( results_cc_age_coef_female, "logistic", identities, "age" ) %>%
  mutate( Exposure = ifelse( grepl( "age_exp", Exposure ) & !grepl( "Male", Exposure ), 
                             paste0( Exposure, "_Female" ), Exposure ) )

results_cc_age_pr_logistic <- rbind( results_cc_age_pr_logistic_male[ c( 1:3 ), ], 
...
writexl::write_xlsx( results_cc_pr_poisson, "results_cc_pr_poisson.xlsx" )
