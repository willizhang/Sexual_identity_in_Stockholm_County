...
### 7. Estimation of Proportion of Change in Sexual Identity
#### 7.1. Complete-case analysis
##### 7.1.1. Define survey design
```{r}
# prepare dataset
...
#### 7.2. Survey analyses of imputed datasets
```{r}
# among demographic subgroups
...
### 8. Relation between Change in Sexual Identity and Demographic Factors
#### 8.1. Complete-case analysis
##### 8.1.1. Define survey design
```{r}
##### prepare dataset #####
...
d_2014_cc_model_fluidity$country_of_birth <- factor( 
  ...
  levels = c( "Sweden", "Europe", "Outside Europe" ) )
d_2014_cc_model_fluidity$country_of_birth_exp <- d_2014_cc_model_fluidity$country_of_birth
...
# living status, and personal support
...
d_2014_cc_model_fluidity$country_of_birth_conf <- d_2014_cc_model_fluidity$country_of_birth # country of birth

d_2014_cc_model_fluidity$education_conf <- d_2014_cc_model_fluidity$education # education

d_2014_cc_model_fluidity$income_conf <- d_2014_cc_model_fluidity$dispink2014 # disposable income

d_2014_cc_model_fluidity$marital_status_conf <- d_2014_cc_model_fluidity$marital_status # marital status

d_2014_cc_model_fluidity$living_status_conf <- d_2014_cc_model_fluidity$living_alone # living status

d_2014_cc_model_fluidity$personal_support_conf <- d_2014_cc_model_fluidity$personal_support # personal support

summary( d_2014_cc_model_fluidity )
...
fml_sexual_identity_model_2 <- sexual_identity_fluidity ~ sexual_identity_2014_exp + age_conf + sex_conf + country_of_birth_conf + education_conf + income_conf + marital_status_conf + living_status_conf + personal_support_conf
...
fml_age_model_2 <- sexual_identity_fluidity ~ age_exp + sex_conf + country_of_birth_conf + education_conf + income_conf + marital_status_conf + living_status_conf + personal_support_conf + sexual_identity_2014_conf
...
fml_age_by_sex_model_2 <- sexual_identity_fluidity ~ age_exp*sex_exp + country_of_birth_conf + education_conf + income_conf + marital_status_conf + living_status_conf + personal_support_conf + sexual_identity_2014_conf
...
fml_sex_model_2 <- sexual_identity_fluidity ~ sex_exp + age_conf + country_of_birth_conf + education_conf + income_conf + marital_status_conf + living_status_conf + personal_support_conf + sexual_identity_2014_conf
...
fml_country_of_birth_model_2 <- sexual_identity_fluidity ~ country_of_birth_exp + age_conf + sex_conf + education_conf + income_conf + marital_status_conf + living_status_conf + personal_support_conf + sexual_identity_2014_conf
...
fml_education_model_2 <- sexual_identity_fluidity ~ education_exp + age_conf + sex_conf + country_of_birth_conf + income_conf + marital_status_conf + living_status_conf + personal_support_conf + sexual_identity_2014_conf
...
fml_income_model_2 <- sexual_identity_fluidity ~ income_exp + age_conf + sex_conf + country_of_birth_conf + education_conf + marital_status_conf + living_status_conf + personal_support_conf + sexual_identity_2014_conf
...
fml_marital_status_model_2 <- sexual_identity_fluidity ~ marital_status_exp + age_conf + sex_conf + country_of_birth_conf + education_conf + income_conf + living_status_conf + personal_support_conf + sexual_identity_2014_conf
...
fml_living_status_model_2 <- sexual_identity_fluidity ~ living_status_exp + age_conf + sex_conf + country_of_birth_conf + education_conf + income_conf + marital_status_conf + personal_support_conf + sexual_identity_2014_conf
...
fml_personal_support_model_2 <- sexual_identity_fluidity ~ personal_support_exp + age_conf + sex_conf + country_of_birth_conf + education_conf + income_conf + marital_status_conf + living_status_conf + sexual_identity_2014_conf
...
##### 8.1.3. Crude and adjusted analyses
```{r}
...
#### 8.2. Survey analyses of imputed datasets
##### 8.2.1. Unadjusted and adjusted analyses
```{r}
# fit Poisson regression model for each exposure
models_fluidity_imp <- list()

for ( exposure in exposures ) {
  models_fluidity_imp[[ exposure ]] <- list()
  
  fml_list_name <- paste( "fml_list", exposure, sep = "_" )
  
  for ( name in names( get( fml_list_name ) ) ) {
    models_fluidity_imp[[ exposure ]][[ name ]] <- fit_model_imp( formula = get( fml_list_name )[[ name ]],
                                                                  design = survey_design_imp,
                                                                  year = 2014 )
  }
}
...
# refit the model for age by sex, using female as the reference group
# to extract results for age among females
refit_model_fluidity_imp_age_by_sex <- list()

for ( name in names( fml_list_age_by_sex ) ) {
  refit_model_fluidity_imp_age_by_sex[[ name ]] <- fit_model_imp( formula = fml_list_age_by_sex[[ name ]],
                                                                  design = survey_design_imp_refit,
                                                                  year = 2014 )
}
```

##### 8.2.2. Extract results
```{r}
exposures <- c( "sexual_identity", "age", "sex", "country_of_birth", "education", "income", "marital_status", "living_status", "personal_support" )
...
########## Crude analyses ##########
fluidity_crude_model_most <- extract_fluidity_model_imp( model_results = models_fluidity_imp,
                                                         exposures = exposures,
                                                         model_type = "crude" )

# among males
fluidity_crude_model_male <- subset(
  models_fluidity_imp[[ "age_by_sex" ]][[ "fml_age_by_sex_crude" ]], subgroup %in% exposures_age )

colnames( fluidity_crude_model_male )[-1] <- paste( colnames( fluidity_crude_model_male )[-1], "crude", sep = "_" )
fluidity_crude_model_male$subgroup <- paste( "male_", fluidity_crude_model_male$subgroup )

# among females
fluidity_crude_model_female <- subset(
  refit_model_fluidity_imp_age_by_sex[[ "fml_age_by_sex_crude" ]], subgroup %in% exposures_age )

colnames( fluidity_crude_model_female )[-1] <- paste( colnames( fluidity_crude_model_female )[-1], "crude", sep = "_" )
fluidity_crude_model_female$subgroup <- paste( "female_", fluidity_crude_model_female$subgroup )

# merge datasets
fluidity_crude_model_summary <- rbind( fluidity_crude_model_most, fluidity_crude_model_male, fluidity_crude_model_female )


########## Model 1 ##########
fluidity_model_1_most <- extract_fluidity_model_imp( model_results = models_fluidity_imp,
                                                     exposures = exposures,
                                                     model_type = "model_1" )

# among males
fluidity_model_1_male <- subset(
  models_fluidity_imp[[ "age_by_sex" ]][[ "fml_age_by_sex_model_1" ]], subgroup %in% exposures_age )

colnames( fluidity_model_1_male )[-1] <- paste( colnames( fluidity_model_1_male )[-1], "model_1", sep = "_" )
fluidity_model_1_male$subgroup <- paste( "male_", fluidity_model_1_male$subgroup )

# among females
fluidity_model_1_female <- subset(
  refit_model_fluidity_imp_age_by_sex[[ "fml_age_by_sex_model_1" ]], subgroup %in% exposures_age )

colnames( fluidity_model_1_female )[-1] <- paste( colnames( fluidity_model_1_female )[-1], "model_1", sep = "_" )
fluidity_model_1_female$subgroup <- paste( "female_", fluidity_model_1_female$subgroup )

# merge datasets
fluidity_model_1_summary <- rbind( fluidity_model_1_most, fluidity_model_1_male, fluidity_model_1_female )


########## Model 2 ##########
fluidity_model_2_most <- extract_fluidity_model_imp( model_results = models_fluidity_imp,
                                                     exposures = exposures,
                                                     model_type = "model_2" )

# among males
fluidity_model_2_male <- subset(
  models_fluidity_imp[[ "age_by_sex" ]][[ "fml_age_by_sex_model_2" ]], subgroup %in% exposures_age )

colnames( fluidity_model_2_male )[-1] <- paste( colnames( fluidity_model_2_male )[-1], "model_2", sep = "_" )
fluidity_model_2_male$subgroup <- paste( "male_", fluidity_model_2_male$subgroup )

# among females
fluidity_model_2_female <- subset(
  refit_model_fluidity_imp_age_by_sex[[ "fml_age_by_sex_model_2" ]], subgroup %in% exposures_age )

colnames( fluidity_model_2_female )[-1] <- paste( colnames( fluidity_model_2_female )[-1], "model_2", sep = "_" )
fluidity_model_2_female$subgroup <- paste( "female_", fluidity_model_2_female$subgroup )

# merge datasets
fluidity_model_2_summary <- rbind( fluidity_model_2_most, fluidity_model_2_male, fluidity_model_2_female )


# export results
fluidity_imp_rr_2014 <- merge( fluidity_crude_model_summary,
                               merge( fluidity_model_1_summary, fluidity_model_2_summary, by = "subgroup"  ),
                               by = "subgroup" )

writexl::write_xlsx( fluidity_imp_rr_2014, "fluidity_imp_rr_2014.xlsx" )
```